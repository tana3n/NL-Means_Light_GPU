//fxc /Tps_3_0 /Eprocess /DTIME_RADIUS=1 /Foeven_t1.pso nlmeans_even_t.hlsl
//fxc /Tps_3_0 /Eprocess /DTIME_RADIUS=2 /Foeven_t2.pso nlmeans_even_t.hlsl
//fxc /Tps_3_0 /Eprocess /DTIME_RADIUS=3 /Foeven_t3.pso nlmeans_even_t.hlsl
...
float4 getWeight(float4 v1, float4 v2, float4 v3, float4 v4, float4 t1, float4 t2, float4 t3, float4 t4)
{
	...
	d1 = t2 - v2;
	d1 *= d1;
	sum2.xy += (d1.xy + d1.zw) * 0.12 + d1.yz * 0.20;
	sum2.zw = (d1.xy + d1.zw) * 0.07 + d1.yz * 0.12;
	d1 = t3 - v3;
	d1 *= d1;
	sum2.xy += (d1.xy + d1.zw) * 0.07 + d1.yz * 0.12;
	sum2.zw += (d1.xy + d1.zw) * 0.12 + d1.yz * 0.20;
	d1 = t4 - v4;
	d1 *= d1;
	sum2.zw += (d1.xy + d1.zw) * 0.07 + d1.yz * 0.12;
	return exp(-4096 * 4096 * sum2 * H2);
}
