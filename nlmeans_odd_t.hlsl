//fxc /Tps_3_0 /Eprocess /DTIME_RADIUS=1 /Foodd_t1.pso nlmeans_odd_t.hlsl
//fxc /Tps_3_0 /Eprocess /DTIME_RADIUS=2 /Foodd_t2.pso nlmeans_odd_t.hlsl
//fxc /Tps_3_0 /Eprocess /DTIME_RADIUS=3 /Foodd_t3.pso nlmeans_odd_t.hlsl
...
float4 process(float2 pos : VPOS) : COLOR0
{
	...
	[unroll]for (int dt=0; dt<TIME_RADIUS*2+1; dt++)
	{
		...
		[loop]for (int i=0; i<range; i++)
		{
			...
			float4 t1, t2, t3, t4;
			float4 w;
			t1 = float4(tex2D(s[dt], pos+delta[0]).xy, tex2D(s[dt], pos+delta[1]).xy);
			t2 = float4(tex2D(s[dt], pos+delta[3]).xy, tex2D(s[dt], pos+delta[4]).xy);
			t3 = float4(tex2D(s[dt], pos+delta[6]).xy, tex2D(s[dt], pos+delta[7]).xy);
			t4 = float4(tex2D(s[dt], pos+delta[9]).xy, tex2D(s[dt], pos+delta[10]).xy);
			...
			{
				float4 t5, t6;
				t5 = float4(tex2D(s[dt], pos+delta[2]).xy, tex2D(s[dt], pos+delta[5]).xy);
				t6 = float4(tex2D(s[dt], pos+delta[8]).xy, tex2D(s[dt], pos+delta[11]).xy);
